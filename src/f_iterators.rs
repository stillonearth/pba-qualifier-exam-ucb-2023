...
pub fn sum_of_squares(vals: impl Iterator<Item = u32>) -> u32 {
	vals.reduce(|acc, e| acc + e * e).unwrap()
}
...
pub fn first_n_even(mut vals: impl Iterator<Item = u32>) -> Option<impl Iterator<Item = u32>> {
	if let Some(max_count) = vals.next() {
		let evens = vals.skip(1).filter(|v| v % 2 == 0);
		let result = evens
			.enumerate()
			.filter(move |(i, _)| *i < max_count as usize)
			.map(|(_, e)| e);
		return Some(result);
	}
	return None;
...
pub fn square_whole_numbers() -> impl Iterator<Item = u32> {
	...
	impl InfiniteCounter {
		fn new() -> InfiniteCounter {
			InfiniteCounter { count: 0 }
		}
	}
...
impl Iterator for Fibonacci {
	...
	fn next(&mut self) -> Option<u32> {
		...
		let new_value = self.prev.unwrap() + self.prev_prev.unwrap();
